/**
 * Star tool - Click and drag to create stars
 */

import { Tool, type ToolContext } from './toolBase';
import type { PointerEventData, StarShape } from '../types';
import { createStar, renderStar } from '../shapes/star';

export class StarTool extends Tool {
  private startX = 0;
  private startY = 0;
  private isDrawing = false;
  private currentShape: StarShape | null = null;
  private stylePreset = {
    strokeColor: '#000000',
    strokeWidth: 2,
  ...
  onPointerDown(event: PointerEventData, context: ToolContext): void {
    ...
    // Create initial star (zero size)
    this.currentShape = createStar(
      this.startX,
      this.startY,
      0,
      0,
  ...
  renderOverlay(ctx: CanvasRenderingContext2D, _context: ToolContext): void {
    if (this.currentShape) {
      ...
      renderStar(ctx, this.currentShape);
    }
  }

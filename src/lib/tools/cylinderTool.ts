/**
 * Cylinder tool - Click and drag to create cylinders
 */

import { Tool, type ToolContext } from './toolBase';
import type { PointerEventData, CylinderShape } from '../types';
import { createCylinder, renderCylinder } from '../shapes/cylinder';

export class CylinderTool extends Tool {
  private startX = 0;
  private startY = 0;
  private isDrawing = false;
  private currentShape: CylinderShape | null = null;
  private stylePreset = {
    strokeColor: '#000000',
    strokeWidth: 2,
  ...
  onPointerDown(event: PointerEventData, context: ToolContext): void {
    ...
    // Create initial cylinder (zero size)
    this.currentShape = createCylinder(
      this.startX,
      this.startY,
      0,
      0,
  ...
  renderOverlay(ctx: CanvasRenderingContext2D, _context: ToolContext): void {
    if (this.currentShape) {
      ...
      renderCylinder(ctx, this.currentShape);
    }
  }

/**
 * Cloud tool - Click and drag to create clouds
 */

import { Tool, type ToolContext } from './toolBase';
import type { PointerEventData, CloudShape } from '../types';
import { createCloud, renderCloud } from '../shapes/cloud';

export class CloudTool extends Tool {
  private startX = 0;
  private startY = 0;
  private isDrawing = false;
  private currentShape: CloudShape | null = null;
  private stylePreset = {
    strokeColor: '#000000',
    strokeWidth: 2,
  ...
  onPointerDown(event: PointerEventData, context: ToolContext): void {
    ...
    // Create initial cloud (zero size)
    this.currentShape = createCloud(
      this.startX,
      this.startY,
      0,
      0,
  ...
  renderOverlay(ctx: CanvasRenderingContext2D, _context: ToolContext): void {
    if (this.currentShape) {
      ...
      renderCloud(ctx, this.currentShape);
    }
  }

/**
 * Diamond tool - Click and drag to create diamonds
 */

import { Tool, type ToolContext } from './toolBase';
import type { PointerEventData, DiamondShape } from '../types';
import { createDiamond, renderDiamond } from '../shapes/diamond';

export class DiamondTool extends Tool {
  private startX = 0;
  private startY = 0;
  private isDrawing = false;
  private currentShape: DiamondShape | null = null;
  private stylePreset = {
    strokeColor: '#000000',
    strokeWidth: 2,
  ...
  onPointerDown(event: PointerEventData, context: ToolContext): void {
    ...
    // Create initial diamond (zero size)
    this.currentShape = createDiamond(
      this.startX,
      this.startY,
      0,
      0,
  ...
  renderOverlay(ctx: CanvasRenderingContext2D, _context: ToolContext): void {
    if (this.currentShape) {
      ...
      renderDiamond(ctx, this.currentShape);
    }
  }
